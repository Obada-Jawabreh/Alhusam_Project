...
const NavigationBar = () => {
  ...
  return (
    <nav className="bg-white/90 border-b border-gray-200 py-4 shadow-sm fixed top-0 left-0 right-0 z-50">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        {/* Logo */}
        <Link to="/" className="flex items-center text-blue-700 font-bold text-xl">
          <img src={customLogo} alt="Custom Logo" className="w-10 h-10 mr-2 object-cover" />
          منصة تسويق
        </Link>
          ...
          <div className="mr-4">
            {isAuthenticated && user ? (
              <div className="relative" ref={dropdownRef}>
                <div
                  onClick={toggleProfileDropdown}
                  className="flex items-center cursor-pointer bg-blue-50 px-3 py-2 rounded-md hover:bg-blue-100 transition-colors duration-300"
                >
                  ...
                  <span className="text-blue-800 mr-2 text-lg">
                    {user.username}
                  </span>
                  <svg
                ...
                {isProfileDropdownOpen && (
                  <div className="absolute left-0 mt-2 w-full min-w-[200px] bg-white rounded-md shadow-lg z-50">
                    <button
                      onClick={handleProfileNavigation}
                      className="block w-full text-right px-4 py-2 text-blue-800 hover:bg-blue-50 rounded-t-md"
                    >
                      الملف الشخصي
                    </button>
                    <button
                      onClick={handleLogout}
                      className="block w-full text-right px-4 py-2 text-blue-800 hover:bg-blue-50 rounded-b-md"
                    >
                      تسجيل الخروج
                    </button>
                  </div>
      ...
      {/* Mobile Menu */}
      {isMenuOpen && (
        <div className="lg:hidden bg-white/95 border-t border-gray-200 py-4 shadow-sm">
          <div className="container mx-auto px-4 sm:px-6 flex flex-col space-y-4">
            <Link
              to="/"
              className="text-blue-600 hover:text-blue-800 transition-colors duration-300"
            ...
            {isAuthenticated && user ? (
              <div>
                <div className="flex items-center mb-4">
                  ...
                  <span className="text-blue-800 text-lg mr-3">{user.username}</span>
                </div>
                <button
                  onClick={handleProfileNavigation}
                  className="block w-full px-4 py-2 text-blue-800 hover:bg-blue-50 rounded-t-md"
                >
                  الملف الشخصي
                </button>
                <button
